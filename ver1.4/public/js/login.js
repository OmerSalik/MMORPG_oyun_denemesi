  ...
  switch (process) {
    case 'login':
      s.emit('process', {process:'login', uname:gia('uname').value.replace("'", '%'), pass:gia('pass').value.replace("'", '%')}, (response, token) => {
        if(response) {
          showMessage(words.login + ' ' + words['success'], 'success');
          f.set('auth', token);
          setTimeout(() => {location = 'game'}, 750);
        } else showMessage(words.user_not_found, 'error')
      });
      break;
    case 'signup':
      if(kia('pass').value == kia('pass-again').value) {
        s.emit('process', {
          ...
          if(res === false) showMessage(words.uname_taken, 'warning');
          else {
            showMessage(words.success, 'success');
            f.set('auth', res);
            setTimeout(() => {location = 'game'}, 750)
          }
        });
      } else showMessage(words.pass_dont_match, 'warning')
      break;
  }
}